...
/**
 * Creates a workflow with multiple steps for testing internal span visibility.
 * This workflow has two sequential steps, allowing us to verify that
 * internal workflow step spans are visible as children of the workflow span.
 */
function createMultiStepProcessorWorkflow(id: string) {
  // Create first step
  ...
  // Create the workflow with two steps
  return createWorkflow({
    id,
    inputSchema: ProcessorStepSchema,
    outputSchema: ProcessorStepSchema,
...
describe('Processor Tracing Tests', () => {
  ...
  describe('Step Processors', () => {
     ...
     *   - MODEL_GENERATION
     *     - MODEL_STEP
     *   - output stream processor: stream-transformer PROCESSOR_RUN
     */
    it('should trace output stream processor with processOutputStream', async () => {
      ...
      const agent = new Agent({
        ...
        instructions: 'Test',
        model,
        outputProcessors: [new OutputStreamProcessor('stream-transformer')],
      });
      ...
      // EXPECTED: 1 agent span, 1 model span, 1 model step spans, and 1 output stream processor span
      expect(agentSpans.length).toBe(1);
      expect(modelSpans.length).toBe(1);
      expect(modelStepSpans.length).toBe(1);
      expect(processorSpans.length).toBe(1);
      ...
      const streamSpan = processorSpans[0];
      expect(streamSpan?.name).toBe('output stream processor: stream-transformer');
      ...
      // 2. MODEL_STEP is child of MODEL_GENERATION
      expect(modelStepSpan?.parentSpanId).toBe(modelSpan?.id);
      // 3. Stream processor span is child of agent span
      expect(streamSpan?.parentSpanId).toBe(agentSpan?.id);
  ...
  describe('Workflow as Processor', () => {
    ...
    /**
     * Expected span structure (multi-step workflow):
     * - test-agent AGENT_RUN (root)
     *   - input processor: multi-step-workflow PROCESSOR_RUN
     *     - WORKFLOW (multi-step-workflow)
     *       - WORKFLOW_STEP (processor:multi-step-workflow-first)
     *       - WORKFLOW_STEP (processor:multi-step-workflow-second)
     *   - MODEL_GENERATION
     *     - MODEL_STEP
     *
     * This test verifies that a workflow with multiple sequential processor steps
     * creates separate workflow step spans for each processor in the chain.
     */
    it('should trace workflow with multiple processor steps showing all internal spans', async () => {
      const model = createMockModel();
      // Create workflow with multiple processor steps
      const processorWorkflow = createMultiStepProcessorWorkflow('multi-step-workflow');
      ...
      // EXPECTED: 1 agent span, 1 model span
      expect(agentSpans.length).toBe(1);
      expect(modelSpans.length).toBe(1);
      ...
      // EXPECTED: Workflow spans should be visible
      expect(workflowSpans.length).toBe(1);

      // Find the multi-step workflow span
      const multiStepWorkflowSpan = workflowSpans.find(s => s.entityId === 'multi-step-workflow');
      expect(multiStepWorkflowSpan).toBeDefined();

      // EXPECTED: Should have 2 workflow step spans (one per processor in the workflow)
      const workflowChildSteps = workflowStepSpans.filter(s => s.parentSpanId === multiStepWorkflowSpan?.id);
      expect(workflowChildSteps.length).toBe(2);

      // Verify step names include the processor IDs
      const stepNames = workflowChildSteps.map(s => s.name);
      expect(stepNames.some(name => name?.includes('first'))).toBe(true);
      expect(stepNames.some(name => name?.includes('second'))).toBe(true);

      // FULL HIERARCHY VERIFICATION:
      // 1. Workflow span is child of agent span
      expect(multiStepWorkflowSpan?.parentSpanId).toBe(agentSpan?.id);
      // 2. MODEL_GENERATION is child of AGENT_RUN
      expect(modelSpan?.parentSpanId).toBe(agentSpan?.id);

      // EXECUTION ORDER: workflow runs before MODEL_GENERATION
      testExporter.expectStartedBefore(multiStepWorkflowSpan, modelSpan);
  ...
  describe('Memory Processors', () => {
    ...
    it('should trace MessageHistory processor when memory is enabled', async () => {
      ...
      expect(messageHistorySpans.length).toBe(1);
    ...
    it('should trace memory processors alongside custom processors', async () => {
      ...
      expect(processorSpans.length).toBe(3);
